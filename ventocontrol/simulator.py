...
def _log(tag: str, addr: tuple, params: Optional[dict] = None, prefix: str = "") -> None:
...
class SimDevice:
    ...
    def __init__(
        self, index: int, password: str = SIM_PASSWORD, unit_type: int = SIM_UNIT_TYPE, id_prefix: str = SIM_ID_PREFIX
    ) -> None:
        self.index = index
    ...
    def _make_initial_state(self, v: dict) -> dict[int, bytes]:
        ...
        return {
            ...
            Param.RTC_CALENDAR: bytes([now.day, now.weekday() + 1, now.month, now.year % 100]),
    ...
    def _tick_rtc(self) -> None:
        ...
        self._state[Param.RTC_CALENDAR] = bytes([now.day, now.weekday() + 1, now.month, now.year % 100])
    ...
    def handle(self, func_byte: int, data: bytes, addr: tuple, sock: socket.socket, tag_prefix: str = "") -> None:
    ...
    def _handle_read(self, _func: int, data: bytes, addr: tuple, sock: socket.socket, tag_prefix: str) -> None:
    ...
    def _handle_write(self, _func: int, data: bytes, addr: tuple, sock: socket.socket, tag_prefix: str) -> None:
        updates = _parse_write_data(data)
        self._apply_writes(updates)
        _log("WRITE", addr, updates, tag_prefix)

    def _handle_write_resp(self, _func: int, data: bytes, addr: tuple, sock: socket.socket, tag_prefix: str) -> None:
        updates = _parse_write_data(data)
        self._apply_writes(updates)
        _log("WRSP ", addr, updates, tag_prefix)
        self._send_response(list(updates.keys()), addr, sock)

    def _handle_increment(self, _func: int, data: bytes, addr: tuple, sock: socket.socket, tag_prefix: str) -> None:
    ...
    def _handle_decrement(self, _func: int, data: bytes, addr: tuple, sock: socket.socket, tag_prefix: str) -> None:
        params = _parse_read_request_data(data)
        for p in params:
            self._nudge(p, -1)
    ...
    def respond_to_discovery(self, requested: list[int], addr: tuple, sock: socket.socket) -> None:
        """Send a discovery response for the given params (called by the server)."""
        self._send_response(requested, addr, sock)

    def _send_response(self, requested: list[int], addr: tuple, sock: socket.socket) -> None:
...
class VentoFanSim:
    ...
    def __init__(
        ...
        id_prefix: str = SIM_ID_PREFIX,
    ) -> None:
        self._port = port
        self._devices = [SimDevice(start_index + i, unit_type=unit_type, id_prefix=id_prefix) for i in range(count)]
    ...
    def _dispatch(self, raw: bytes, addr: tuple) -> None:
        ...
        try:
            ...
            data_start, target_id, pwd, func_byte = (
                header.data_start,
                header.device_id,
                header.password,
                header.func_byte,
        ...
        if target_id_bytes == DEFAULT_DEVICE_ID:
            ...
            if dev:
                dev.handle(func_byte, data, addr, self._sock, tag_prefix=f"#{dev.index + 1}")
